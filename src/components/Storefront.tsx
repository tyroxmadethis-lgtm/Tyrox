...
export const Storefront: React.FC<StorefrontProps> = ({
  ...
  const setSearchQuery =
    setExternalSearchQuery !== undefined
  ...
  const [checkoutReceipt, setCheckoutReceipt] = useState<{
    orderId: string;
    totalPaid: number;
    tracks: Array<{ title: string; license: string }>;
  ...
  const [activePlatform, setActivePlatform] = useState<"native" | "airbit">(
    "native",
  );
  const [airbitUrl, setAirbitUrl] = useState(() => {
    return (
      localStorage.getItem("tyrox_airbit_url") ||
      "https://airbit.com/widgets/html5?uid=12354&config=301389"
    );
  });
  ...
  const [profileImg, setProfileImg] = useState(() => {
    return (
      localStorage.getItem("tyrox_profile_img") ||
      "/static/images/tyrox_profile.jpg"
    );
  });
  ...
  useEffect(() => {
        ...
        if (data && data.length > 0) {
          const mapped: Track[] = data.map((t: any, idx: number) => {
            ...
            const streamUrl =
              t.stream_url ||
              t.audioUrl ||
              "/static/converted/god_mode_tagged_preview.mp3";
            const coverArt =
              t.cover_art_url ||
              t.image_url ||
              t.imageUrl ||
          ...
          console.log(
            "⚡ Success! Storefront layout completely synced with your direct cloud vault.",
          );
        }
      } catch (err) {
        console.warn(
          "Could not load cloud assets, using local fallback state loops:",
          err,
        );
      } finally {
        setLoadingSupabase(false);
    ...
    (window as any).loadTrackIntoBlazeDeck = (
      audioStreamUrl: string,
      trackTitle: string,
    ) => {
      const audioEngine = document.getElementById(
        "nativeAudioEngine",
      ) as HTMLAudioElement;
      ...
      if (audioEngine && playerTitleDisplay) {
        ...
        audioEngine
          .play()
          .catch((e) =>
            console.log("Stream init delayed until interaction:", e),
          );
      }
    };
    ...
    if (file) {
      ...
      reader.onload = () => {
        ...
        window.dispatchEvent(
          new CustomEvent("tyrox-banner-updated", { detail: dataUrl }),
        );
      };
  ...
  const handleProfileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = () => {
        ...
        window.dispatchEvent(
          new CustomEvent("tyrox-profile-updated", { detail: dataUrl }),
        );
      };
      reader.readAsDataURL(file);
    }
  ...
  const handleFreeDownloadSubmit = (e: React.FormEvent) => {
    ...
    const existing = JSON.parse(
      localStorage.getItem("vv_newsletter_subscribers") || "[]",
    );
    ...
    apiFetch(
      `/api/v1/store/track-free-download?track_name=${encodeURIComponent(downloadTrack.title)}&buyer_email=${encodeURIComponent(downloadEmail)}`,
      {
        method: "POST",
      },
    ).catch((err) =>
      console.error("Error logging free download tracking:", err),
    );
  ...
  const [activeQuickPriceTrackId, setActiveQuickPriceTrackId] = useState<
    string | null
  >(null);
  ...
  const allTags = [
    "All",
    ...Array.from(new Set(tracks.flatMap((t) => t.tags))),
  ];
  ...
  const handlePayPalSuccess = (paypalOrderId: string) => {
    ...
    cart.forEach((item) => {
      ...
      apiFetch(
        `/api/v1/store/track-paid-purchase?track_name=${encodeURIComponent(item.track.title)}&license_type=${encodeURIComponent(item.license.name)}&buyer_email=${encodeURIComponent(email || "label_client@universal.com")}&payout_amount=${finalPrice}`,
        {
          method: "POST",
        },
      ).catch((err) =>
        console.error("Error logging paid purchase tracking:", err),
      );
    });

    const res = checkout(
      email || "label_client@universal.com",
      activeCouponCode,
    );
    if (res.success) {
      setCheckoutReceipt({
    ...
    try {
      ...
      const checkoutPromises = cart.map(async (item) => {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errData.error || `Checkout failed for track: ${item.track.title}`,
          );
        }
      ...
      cart.forEach((item) => {
        ...
        apiFetch(
          `/api/v1/store/track-paid-purchase?track_name=${encodeURIComponent(item.track.title)}&license_type=${encodeURIComponent(item.license.name)}&buyer_email=${encodeURIComponent(email)}&payout_amount=${finalPrice}`,
          {
            method: "POST",
          },
        ).catch((err) =>
      ...
      if (res.success) {
        setCheckoutReceipt({
          orderId:
            lastResult?.legalContract?.digitalSignatureHash ||
            res.orderId ||
            "pi_stripe_tx",
          totalPaid,
          tracks: itemsSummary,
  ...
  const handleQuickAdd = (
    track: Track,
    tierId: "mp3" | "wav" | "unlimited" | "exclusive",
  ) => {
  ...
  return (
    <div
      id="store-main-viewport"
      className="pb-32 px-4 md:px-8 max-w-7xl mx-auto pt-6 flex flex-col gap-6 md:gap-8 min-h-screen bg-bg-dark text-neutral-100"
    >
      ...
      {activePlatform === "airbit" && (
        <div
          id="airbit-frame-wrapper"
          className="storefront-viewport mb-12 animate-fadeIn text-left"
        >
          <div className="flex items-center justify-between mb-4 bg-neutral-950/60 border border-neutral-900 rounded-xl p-4">
            <div>
              <h3 className="text-sm font-sans font-extrabold text-white uppercase tracking-wider flex items-center gap-2">
                <span className="text-emerald-550 font-bold">💼</span> Airbit
            ...
            <button
              onClick={() => {
                const newUrl = prompt(
                  "Enter your custom Airbit Widgets HTML5 URL:",
                  airbitUrl,
                );
                if (newUrl) {
      ...
      {activePlatform === "native" && (
        <>
          ...
          >
            ...
            <div className="hero-content-alignment-box text-center">
              ...
              <div className="hero-search-hub max-w-lg mx-auto shadow-2xl transition duration-300 hover:scale-[1.01] hover:shadow-purple-500/10 border border-neutral-200/10 mb-6 bg-white/95">
                <span className="text-neutral-500 font-bold select-none text-base">
                  🔍
                </span>
                <input
                  type="text"
              ...
              {tracks.length > 0 && (
                ...
                >
                  <span className="ticker-badge text-[#a855f7] uppercase font-mono tracking-widest font-extrabold select-none">
                    🔥 TRENDING:
                  </span>
                  <span
                    id="tickerTrackTitle"
                    className="ticker-track-text text-neutral-100 font-bold uppercase truncate"
                  >
                    {tracks[0].title}
                  </span>
                  <span className="ticker-play-indicator text-[11px] font-mono text-purple-400 font-bold uppercase tracking-widest pl-2">
                    ▶ Click to Preview
                  </span>
                </div>
              )}
            ...
            <div className="mixes-grid-canvas" id="liveMixesCanvas">
              {streamEligibleMixes.length === 0 ? (
                <div className="col-span-full border border-dashed border-neutral-800/80 bg-neutral-950/30 rounded-2xl p-8 text-center my-2 max-w-full">
                  ...
                  <h3 className="font-sans font-extrabold text-neutral-200 uppercase tracking-wider text-xs">
                    No Mixes Accumulated Yet
                  </h3>
                  <p className="font-mono text-[10px] text-neutral-500 uppercase tracking-widest max-w-md mx-auto mt-2 leading-relaxed">
                    As soon as any of your beats get streamed or previewed, they
                    will automatically be compiled right here as high-fidelity
                    mixes in real-time.
                  </p>
                  <button
                    onClick={() => {
                      ...
                      if (catalogSect)
                        catalogSect.scrollIntoView({ behavior: "smooth" });
                    }}
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-1 mt-2">
                        <div className="flex items-center justify-between gap-1 w-full">
                          <span className="mix-card-title group-hover:text-purple-400 transition truncate text-xs font-bold text-neutral-100">
                            {track.title} Mix
                          </span>
                        ...
                        <div className="flex items-center justify-between gap-1 w-full mt-1">
                          <p className="font-mono text-[9.5px] text-neutral-500 uppercase tracking-wider">
                            {track.bpm} BPM • {track.key} • PROD. TYROX
                          </p>
                          <span className="stream-badge-ui font-mono text-[9px] text-purple-400 bg-purple-950/10 px-1.5 py-0.5 rounded border border-purple-500/10 select-none">
                            📊 {track.bpm} BPM
                          </span>
                        </div>
                      </div>
          ...
          <section
            id="trendiest-section"
            className="collection-wrapper border-b border-neutral-900/60 pb-6 my-2"
          >
            ...
            >
              {trendiestBeatsList.length === 0 ? (
                <div className="w-full text-center py-6 px-4 rounded-xl border border-dashed border-neutral-900 bg-neutral-950/20 my-1 select-none">
                  ...
                  <p className="text-[10px] font-sans font-extrabold text-neutral-400 uppercase tracking-widest">
                    No plays accumulated yet
                  </p>
                  <p className="text-[9.5px] font-mono text-neutral-600 uppercase tracking-wider mt-1">
                    Play or stream your uploaded beats to send them directly to
                ...
                trendiestBeatsList.map((beat) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <h3 className="font-sans font-bold text-xs text-neutral-100 truncate mt-1">
                        {beat.title}
                      </h3>
                      <p className="font-mono text-[9.5px] text-neutral-500 truncate mt-0.5">
                        {beat.producer}
                      </p>

                      <div className="flex items-center justify-between mt-3 pt-2.5 border-t border-neutral-900">
                        <span className="font-mono text-[10.5px] font-bold text-purple-400">
                          ${beat.price}
                        </span>
                        <span className="font-mono text-[9px] text-neutral-500 bg-neutral-950 px-2 py-0.5 rounded border border-neutral-900/60">
                          {beat.bpm} BPM
                        </span>
                      </div>
                    </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-8 flex flex-col gap-4">
              ...
              {tracks.length > 0 && (
                <div className="bg-[#0b0c10] border border-neutral-900/60 rounded-2xl p-5 mb-2 shadow-2xl">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-[10px] font-mono text-[#39FF14] uppercase tracking-wider font-extrabold">
                      Spotlight Preview Stream
                    </span>
                  </div>
                  <p className="text-xs text-neutral-400 mb-4 font-sans leading-relaxed">
                    Stream our current prime acoustic trap master straight from
                    the Madison Wisconsin vault using our premium native stream
                    player.
                  </p>
                  <BeatStreamPlayer
              ...
              <div className="space-y-2">
                {viewMode === "grid" ? (
                  ...
                  <div
                    id="empty-store-grid"
                    className="p-16 text-center rounded-2xl bg-panel-bg border border-neutral-900"
                  >
                    <Music
                      size={28}
                      className="mx-auto text-neutral-600 mb-2 animate-bounce"
                    />
                    <p className="text-neutral-400 font-sans text-xs uppercase tracking-wider font-bold">
                      No active beats found
                    </p>
                    <p className="text-neutral-500 font-sans text-[11px] mt-1">
                      Please add custom tracks from the Studio Dashboard or sync
                      local data.
                    </p>
                    {tracks.length === 0 ? (
                      <button
                          ...
                          <div
                            ...
                            className={`track-row-item ${
                              currentTrack?.id === track.id
                                ? "active-preview active-highlight"
                                : ""
                            }`}
                          >
                            {/* Play trigger button */}
                            ...
                            {/* Title block with metadata & cover art thumbnail next to it */}
                            <div className="flex items-center gap-3 text-left">
                              <img 
                                src={track.imageUrl} 
                                alt={track.title} 
                                loading="lazy"
                                className="w-10 h-10 object-cover rounded border border-neutral-800 shrink-0 select-none shadow"
                                onError={(e) => {
                                  (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?q=80&w=150&auto=format&fit=crop';
                                }}
                                referrerPolicy="no-referrer"
                              />
                            ...
                            {/* Tags */}
                            <span className="t-tags truncate">
                              {track.tags.join(", ")}
                            </span>
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              <div className="p-5 rounded-2xl bg-panel-bg border border-neutral-900 flex flex-col gap-4 relative">
                ...
                <div className="flex items-center justify-between border-b border-neutral-900 pb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="font-sans font-bold text-xs text-neutral-200 tracking-wide uppercase">
                      BeatStore Cart
                    </h2>
                  </div>
                  <span className="font-mono text-[9.5px] bg-neutral-950 px-2 py-0.5 rounded text-neutral-400">
                    {cart.length} ITEMS
                  </span>
                </div>
                  ...
                  <div className="p-4 rounded-xl bg-cyan-950/20 border border-cyan-500/30 text-cyan-400 space-y-3 animate-scaleUp">
                    ...
                    <p className="text-[10px] text-neutral-400 font-sans leading-relaxed">
                      ...
                      <b className="text-cyan-300 font-mono">
                        {checkoutReceipt.orderId}
                      </b>
                      . Contracts and stems are compiled and delivered.
                    </p>
                    ...
                    {checkoutReceipt.downloadSecureUrl && (
                      <div className="p-2.5 bg-neutral-950 border border-cyan-500/15 rounded-lg flex flex-col gap-1.5">
                        <span className="font-mono text-[8.5px] uppercase text-neutral-400">
                          Direct Delivery Token
                        </span>
                        <a
                    ...
                    {checkoutReceipt.legalContract && (
                      <div className="p-2.5 bg-neutral-950 border border-purple-500/15 rounded-lg space-y-1.5 font-mono text-[9px] text-neutral-400">
                        <span className="text-[8px] uppercase font-black text-purple-400">
                          🛡️ Legal Smart Contract signed
                        </span>
                        <div className="space-y-0.5">
                          <p>
                            License:{" "}
                            <b className="text-neutral-200">
                              {checkoutReceipt.legalContract.licenseType}
                            </b>
                          </p>
                          <p>
                            Issued To:{" "}
                            <b className="text-neutral-200">
                              {checkoutReceipt.legalContract.issuedTo}
                            </b>
                          </p>
                          <p>
                            Signature Hash:{" "}
                    ...
                    <div className="border-t border-cyan-500/10 pt-2 space-y-1">
                      {checkoutReceipt.tracks.map((ct, idx) => (
                        ...
                        >
                          <span className="text-neutral-300 truncate max-w-[140px] font-sans font-medium">
                            {ct.title}
                          </span>
                ...
                {cart.length === 0 ? (
                  <div className="py-12 text-center text-neutral-500 font-sans text-xs">
                    ...
                    Use the ADD buttons to check beats out.
                  </div>
                ) : (
                  <>
                    {/* Cart Items List */}
                    <div className="max-h-56 overflow-y-auto space-y-2 pr-1">
                      {cart.map((item, idx) => (
                        <div
                          key={idx}
                          className="p-3 bg-neutral-950/60 border border-neutral-900 hover:border-neutral-850 rounded-xl flex items-center justify-between gap-3 text-xs animate-fadeIn"
                        >
                          <div className="min-w-0">
                            <p className="font-sans font-bold text-neutral-200 truncate">
                              {item.track.title}
                            </p>
                            <p className="font-mono text-[9px] text-accent-green mt-0.5 uppercase tracking-wide">
                              {item.license.name}
                            </p>
                            <p className="font-mono text-[8.5px] text-[#555] mt-0.5 uppercase">
                              {item.license.format}
                            </p>
                          </div>
                          <div className="flex items-center gap-2 flex-shrink-0">
                            ...
                            <button
                              onClick={() =>
                                removeFromCart(item.track.id, item.license.id)
                              }
                    ...
                    <div className="border-t border-neutral-900 pt-3 space-y-2 text-xs text-neutral-400">
                      ...
                      {couponMessage && (
                        <p
                          className={`text-[10px] font-sans ${couponMessage.error ? "text-red-400" : "text-accent-green"}`}
                        >
                          {couponMessage.text}
                        </p>
                      )}
                    ...
                    {!showCheckoutForm ? (
                      ...
                      <form
                        ...
                        className="space-y-4 pt-4 border-t border-neutral-900 animate-slideIn"
                      >
                        <div className="flex items-[#10111a] justify-between">
                          <div className="flex items-center gap-1.5">
                            <span className="text-sm">💳</span>
                            <h4 className="font-sans font-bold text-xs uppercase text-neutral-200">
                              Stripe Live Checkout
                            </h4>
                          </div>
                          <span className="font-mono text-[11px] text-accent-green font-bold">
                            ${cartTotal.toFixed(2)}
                          </span>
                        </div>

                        <p className="text-[10px] text-neutral-500 font-sans leading-relaxed">
                          Powered by Stripe Connect destination charges. Funds
                          are split and instantly transferred to{" "}
                          <b>{payoutEmail || "tyroxmadethis@gmail.com"}</b>.
                        </p>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-1">
                            <label className="font-mono text-[9px] uppercase text-neutral-500 block text-left">
                              Cardholder Name
                            </label>
                            <input
                              type="text"
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="relative">
                              ...
                              />
                              <span className="absolute right-3 top-2 text-neutral-500 text-xs">
                                🔒
                              </span>
                            </div>
                          </div>
                      ...
                      <form
                        ...
                        className="space-y-3 pt-3 border-t border-neutral-900 animate-slideIn"
                      >
                        <p className="text-[10px] text-neutral-500 font-sans leading-relaxed">
                          Enter email for contract/download issuance. All
                          processing runs fully isolated.
                        </p>
                        <div className="space-y-1">
                          <label className="font-mono text-[9px] uppercase text-neutral-500 block">
                            Payout Delivery Email
                          </label>
                          <input
                            type="email"
                            required
              ...
              {downloadTrack && (
                <FreeDownloadGateModal
                  ...
                  onActivationSuccess={(email) => {
                    ...
                    // Real-time backend tracking: auto-trigger free acquisition ledger logging
                    apiFetch(
                      `/api/v1/store/track-free-download?track_name=${encodeURIComponent(downloadTrack.title)}&buyer_email=${encodeURIComponent(email)}`,
                      {
                        method: "POST",
                      },
                    ).catch((err) =>
                      console.error(
                        "Error logging free download tracking:",
                        err,
                      ),
                    );
                    ...
                    try {
                      const downloadUrl =
                        downloadTrack.audioUrl ||
                        "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3";
                      const link = document.createElement("a");
                      link.href = downloadUrl;
                      link.setAttribute(
                        "download",
                        `${downloadTrack.title}_demo.mp3`,
                      );
                      ...
                      console.error(
                        "Direct browser download trigger failed:",
                        dlErr,
                      );
                    }
                  }}
              ...
              <div className="p-6 rounded-xl bg-[#15161e] border border-[#222533] mb-5 space-y-4 shadow-lg text-left">
                ...
                <ul className="text-white text-[11px] list-none p-0 m-0 space-y-3 leading-relaxed">
                  <li className="flex items-start gap-3">
                    <span className="text-[#9d4edd] font-black text-sm leading-none flex-shrink-0">
                      •
                    </span>
                    <p className="font-sans m-0 text-neutral-200">
                      ...
                      Keep 100% of your earnings on Spotify, Apple Music, and
                      streaming services.
                    </p>
                  </li>
                  <li className="flex items-start gap-3">
                    ...
                    <p className="font-sans m-0 text-neutral-200">
                      <strong className="text-white font-bold">
                        No content claim blocks:
                      </strong>{" "}
                      Direct whitelist protection on platforms prevents
                      copyright problems instantly.
                    </p>
                  </li>
                </ul>
              </div>
              ...
              <div className="audio-card text-left">
                <h3 className="font-sans font-extrabold text-xs text-white uppercase tracking-wider mb-1 animate-pulse">
                  Beat Player Engine
                </h3>
                <p className="text-[10px] sm:text-[10.5px] font-mono text-neutral-400 m-0 leading-normal">
                  High-Fidelity lossy/lossless streaming stream-deck router.
                  Experience Wisconsin studio audio directly.
                </p>
                  ...
                  <div className="min-w-0">
                    <p className="font-sans font-bold text-[11px] text-[#39FF14] truncate uppercase tracking-tight">
                      ...
                      {currentTrack && isPlaying
                        ? "Streaming Vault Audio"
                        : "Engine Online"}
                    </p>
                    <p className="font-mono text-[9px] text-neutral-500 m-0 truncate">
                      {currentTrack
                        ? `Track: ${currentTrack.title}`
                        : "Select any track from below catalog"}
                    </p>
                  </div>
                  <span
                    className={`w-2 h-2 rounded-full ${isPlaying ? "bg-[#39FF14] animate-ping" : "bg-[#39FF14]/40"}`}
                  />
                </div>
                <button
                  id="playTrigger"
                  ...
                  onClick={() => {
                    if (currentTrack) {
                      ...
                      const fallbackTrack =
                        tracks[0] || trendiestBeatsList[0] || null;
                      if (fallbackTrack) {
